...
impl<T: Primitive, P: Pixel<T>, I: GenericImage<P>> ImageOps for I {
        /// Invert the colors of this image.
        /// This method operates inplace.
        fn invert(&mut self) {
        ...
        /// Returns a new image. Does not preserve aspect ratio.
        ///```nwidth``` and ```nheight``` are the new image's dimensions
        fn resize_exact(&self, nwidth: u32, nheight: u32, filter: sample::FilterType) -> ImageBuf<P> {
        ...
        /// Returns a new image. The image's aspect ratio is preserved.
        ///```nwidth``` and ```nheight``` are the new image's dimensions
        fn resize(&self, nwidth: u32, nheight: u32, filter: sample::FilterType) -> ImageBuf<P> {
                let (width, height) = self.dimensions();
        ...
        /// Perfomrs a Gausian blur on this image.
        /// ```sigma``` is a meausure of how much to blur by.
        fn blur(&self, sigma: f32) -> ImageBuf<P> {
        ...
        /// ```threshold``` is a control of how much to sharpen.
        /// see https://en.wikipedia.org/wiki/Unsharp_masking#Digital_unsharp_masking
        fn unsharpen(&self, sigma: f32, threshold: i32) -> ImageBuf<P> {
                sample::unsharpen(self, sigma, threshold)
        }

        /// Filters this image with the specified 3x3 kernel.
        fn filter3x3(&self, kernel: &[f32]) -> ImageBuf<P> {
                if kernel.len() != 9 {
        ...
        /// ```contrast``` is the amount to adjust the contrast by.
        /// Negative values decrease the constrast and positive values increase the constrast.
        fn adjust_contrast(&self, c: f32) -> ImageBuf<P> {
        ...
        /// ```value``` is the amount to brighten each pixel by.
        /// Negative values decrease the brightness and positive values increase it.
        fn brighten(&self, value: i32) -> ImageBuf<P> {
                colorops::brighten(self, value)
        }

        ///Flip this image vertically
        fn flipv(&self) -> ImageBuf<P> {
                affine::flip_vertical(self)
        }

        ///Flip this image horizontally
        fn fliph(&self) -> ImageBuf<P> {
                affine::flip_horizontal(self)
        }

        ///Rotate this image 90 degrees clockwise.
        fn rotate90(&self) -> ImageBuf<P> {
                affine::rotate90(self)
        }

        ///Rotate this image 180 degrees clockwise.
        fn rotate180(&self) -> ImageBuf<P> {
                affine::rotate180(self)
        }

        ///Rotate this image 270 degrees clockwise.
        fn rotate270(&self) -> ImageBuf<P> {
